...
{
	char binFile[1024];
	char *lastDot = binFile + strlen(a_File);
	char *pos = binFile;

	strcpy( binFile, a_File );

	while (strstr( pos + 1, "." )) 
		lastDot = pos = strstr( pos + 1, "." );
	*lastDot = 0;
	...
	else
	{
		...
		for( int i = 0; i < PALETTE_LEVELS; i++ ) 
			fwrite( palette[i], 256 * sizeof( Pixel ), 1, f );
...
{
	if ((a_X < -m_Width) || (a_X > (a_Target->GetWidth() + m_Width)))
		return;

	if ((a_Y < -m_Height) || (a_Y > (a_Target->GetHeight() + m_Height)))
		return;
	...
	if (x2 > a_Target->GetWidth()) 
		x2 = a_Target->GetWidth();
	...
	if (y2 > a_Target->GetHeight()) 
		y2 = a_Target->GetHeight();
